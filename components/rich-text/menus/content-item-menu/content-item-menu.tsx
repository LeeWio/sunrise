...
export function ContentItemMenu() {
  ...
  const { nodeType, alignment, blockStats, isMatch } = useContentItemState(
    editor,
    currentNode,
    searchQuery,
  );
    ...
    async (key: React.Key) => {
      switch (key) {
        case "reset":
          ...
          break;
        case "copy":
          await actions.copyNodeToClipboard();
          break;
        case "delete":
          actions.deleteNode();
          break;
        ...
        case "center":
        case "right":
          actions.setTextAlign(key as string);
          break;
        case "paragraph":
  ...
  const isGeneralVisible =
    ...
    isMatch("Duplicate");
  const isTurnIntoVisible =
    isMatch("Text") ||
    isMatch("Heading 1") ||
    isMatch("Heading 2") ||
    isMatch("Heading 3") ||
    isMatch("Bullet List") ||
    isMatch("Ordered List");
  const isAlignmentVisible =
    ...
    >
      <Dropdown trigger="longPress" onOpenChange={setMenuOpen}>
        ...
        <Dropdown.Popover
          ...
          className="scrollbar-hide flex max-h-[80vh] min-w-[260px] flex-col overflow-y-auto"
        >
          <SearchField
            aria-label="Search actions"
            variant="secondary"
            value={searchQuery}
            className="sticky p-2"
          ...
          <Dropdown.Menu onAction={handleAction} aria-label="Block actions" className="p-1">
              ...
              <Dropdown.Section>
                ...
                {isMatch("Reset Formatting") && (
                  <Dropdown.Item id="reset" textValue="Reset Formatting">
                    <div className="flex items-center gap-2">
                      <ArrowRotateLeft className="text-default-500 size-4" />
                ...
                {isMatch("Copy to Clipboard") && (
                  <Dropdown.Item id="copy" textValue="Copy to Clipboard">
                    <div className="flex w-full items-center gap-2">
                      <Copy className="text-default-500 size-4" />
                ...
                {isMatch("Insert Block") && (
                  <Dropdown.SubmenuTrigger>
                    <Dropdown.Item id="insert-submenu" textValue="Insert Block">
                      <div className="flex items-center gap-2">
                        <Plus className="text-default-500 size-4" />
                    ...
                    <Dropdown.Popover>
                      <Dropdown.Menu onAction={handleAction}>
                        <Dropdown.Item id="add-before" textValue="Add Above">
                          <div className="flex items-center gap-2">
                            <ArrowUp className="text-default-500 size-4" />
                            <Label>Add Above</Label>
                          </div>
                        </Dropdown.Item>
                        <Dropdown.Item id="add-below" textValue="Add Below">
                          <div className="flex items-center gap-2">
                            <ArrowDown className="text-default-500 size-4" />
                            <Label>Add Below</Label>
                          </div>
                        </Dropdown.Item>
                      </Dropdown.Menu>
                    </Dropdown.Popover>
                ...
                {isMatch("Duplicate") && (
                  <Dropdown.Item id="duplicate" textValue="Duplicate">
                    <div className="flex w-full items-center gap-2">
                      <CopyPlus className="text-default-500 size-4" />
            ...
            {isGeneralVisible && (isTurnIntoVisible || isAlignmentVisible || isDangerVisible) && (
              <Separator />
            )}
              ...
              <Dropdown.Section selectionMode="single" selectedKeys={nodeType}>
                ...
                {isMatch("Text") && (
                  <Dropdown.Item id="paragraph" textValue="Text">
                    ...
                    <div className="flex items-center gap-2">
                      <TextIcon className="text-default-500 size-4" />
                      <Label>Text</Label>
                    </div>
                  </Dropdown.Item>
                )}
                {isMatch("Heading 1") && (
                  <Dropdown.Item id="h1" textValue="Heading 1">
                    <Dropdown.ItemIndicator />
                    <div className="flex items-center gap-2">
                      <Heading1 className="text-default-500 size-4" />
                ...
                {isMatch("Heading 2") && (
                  <Dropdown.Item id="h2" textValue="Heading 2">
                    <Dropdown.ItemIndicator />
                    <div className="flex items-center gap-2">
                      <Heading2 className="text-default-500 size-4" />
                ...
                {isMatch("Heading 3") && (
                  <Dropdown.Item id="h3" textValue="Heading 3">
                    <Dropdown.ItemIndicator />
                    <div className="flex items-center gap-2">
                      <Heading3 className="text-default-500 size-4" />
                      <Label>Heading 3</Label>
                    </div>
                  </Dropdown.Item>
                )}
                {isMatch("Bullet List") && (
                  <Dropdown.Item id="bulletList" textValue="Bullet List">
                    <Dropdown.ItemIndicator />
                    <div className="flex items-center gap-2">
                      <ListUl className="text-default-500 size-4" />
                      <Label>Bullet List</Label>
                    </div>
                  </Dropdown.Item>
                )}
                {isMatch("Ordered List") && (
                  <Dropdown.Item id="orderedList" textValue="Ordered List">
                    <Dropdown.ItemIndicator />
                    <div className="flex items-center gap-2">
                      <ListOl className="text-default-500 size-4" />
              ...
              <Dropdown.Section selectionMode="single" selectedKeys={alignment}>
                ...
                {isMatch("Align Left") && (
                  <Dropdown.Item id="left" textValue="Align Left">
                    ...
                    <div className="flex items-center gap-2">
                      <TextAlignLeft className="text-default-500 size-4" />
                      <Label>Align Left</Label>
                    </div>
                  </Dropdown.Item>
                )}
                {isMatch("Align Center") && (
                  <Dropdown.Item id="center" textValue="Align Center">
                    <Dropdown.ItemIndicator type="dot" />
                    <div className="flex items-center gap-2">
                      <TextAlignCenter className="text-default-500 size-4" />
                      <Label>Align Center</Label>
                    </div>
                  </Dropdown.Item>
                )}
                {isMatch("Align Right") && (
                  <Dropdown.Item id="right" textValue="Align Right">
                    <Dropdown.ItemIndicator type="dot" />
                    <div className="flex items-center gap-2">
                      <TextAlignRight className="text-default-500 size-4" />
                ...
                <Dropdown.Section>
                  ...
                  >
                    <div className="flex flex-col gap-1">
                      ...
                      <div className="text-default-500 ml-5 flex gap-3 text-xs">
                        <span>{blockStats.characters} chars</span>
                        <span>{blockStats.words} words</span>
                      </div>
                    </div>
                  </Dropdown.Item>
                </Dropdown.Section>
            ...
            {isDangerVisible &&
              (isGeneralVisible || isTurnIntoVisible || isAlignmentVisible || isInfoVisible) && (
                <Separator />
              )}
