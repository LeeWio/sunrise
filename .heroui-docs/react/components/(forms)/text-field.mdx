...
function CustomTextField() {
  return (
    <TextField className="border-border/60 bgsurface gap-2 rounded-xl border p-4 shadow-sm">
      <Label className="text-default-700 text-sm font-semibold">Project name</Label>
      <Input className="border-border/60 bgsurface rounded-lg border px-3 py-2" />
      <Description className="text-default-500 text-xs">Keep it short and memorable.</Description>
    </TextField>
  );
}
```
