...
function CustomColorField() {
  return (
    <ColorField className="border-border/60 bg-surface gap-2 rounded-xl border p-4 shadow-sm">
      <Label className="text-default-700 text-sm font-semibold">Brand Color</Label>
      <ColorField.Group className="border-border/60 bg-surface rounded-lg border px-3 py-2">
        <ColorField.Prefix>
...
@layer components {
  ...
  .color-input-group__suffix {
    @apply text-field-placeholder flex shrink-0 items-center;
  }
}
```
