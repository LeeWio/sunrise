...
function CustomCalendar() {
  return (
    <Calendar
      ...
      className="border-border bg-surface w-72 rounded-2xl border p-3 shadow-sm"
    >
...
@layer components {
  .calendar {
    @apply border-border bg-surface w-72 rounded-2xl border p-3 shadow-sm;
  }

  .calendar__heading {
    @apply text-default-700 text-sm font-semibold;
  }
