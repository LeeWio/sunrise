...
function CustomTimeField() {
  return (
    <TimeField className="border-border/60 bg-surface gap-2 rounded-xl border p-4 shadow-sm">
      <Label className="text-default-700 text-sm font-semibold">Appointment time</Label>
      <TimeField.Group className="border-border/60 bg-surface rounded-lg border px-3 py-2">
        <TimeField.Input>{(segment) => <TimeField.Segment segment={segment} />}</TimeField.Input>
      </TimeField.Group>
...
@layer components {
  .date-input-group {
    @apply rounded-field bg-field text-field-foreground shadow-field inline-flex h-9 items-center overflow-hidden border text-sm outline-none;
  ...
  .date-input-group__suffix {
    @apply text-field-placeholder pointer-events-none flex shrink-0 items-center;
  }
}
```
