...
function CustomRangeCalendar() {
  return (
    <RangeCalendar
      ...
      className="border-border bg-surface w-80 rounded-2xl border p-3 shadow-sm"
    >
...
@layer components {
  .range-calendar {
    @apply border-border bg-surface w-80 rounded-2xl border p-3 shadow-sm;
  }

  .range-calendar__heading {
    @apply text-default text-sm font-semibold;
  }
