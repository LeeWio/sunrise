...
function CustomDateField() {
  return (
    <DateField className="border-border/60 bg-surface gap-2 rounded-xl border p-4 shadow-sm">
      <Label className="text-default-700 text-sm font-semibold">Appointment date</Label>
      <DateField.Group className="border-border/60 bg-surface rounded-lg border px-3 py-2">
        <DateField.Input>{(segment) => <DateField.Segment segment={segment} />}</DateField.Input>
      </DateField.Group>
