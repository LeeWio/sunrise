...
function CustomSlider() {
  return (
    <Slider className="w-full">
      ...
      <Slider.Track className="bg-surface-secondary h-2 rounded-full">
        <Slider.Fill className="bg-accent" />
        <Slider.Thumb className="bg-accent size-4 rounded-full" />
      </Slider.Track>
    </Slider>
  );
}
...
@layer components {
  ...
  .slider-track {
    @apply bg-surface-secondary relative h-2 w-full rounded-full;
  }
  ...
  .slider-thumb {
    @apply bg-accent border-background size-4 rounded-full border-2;
  }
}
```
...
<Slider defaultValue={60} formatOptions={{ style: "currency", currency: "USD" }}>
  <Label>Price</Label>
  <Slider.Output />
  <Slider.Track>
    <Slider.Fill />
