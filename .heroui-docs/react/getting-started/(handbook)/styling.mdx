...
export function CustomButton({ intent, size, className, ...props }: CustomButtonProps) {
  return <HeroButton className={customButtonVariants({ intent, size, className })} {...props} />;
}
```
...
```tsx
<Button className="px-3 text-sm md:px-4 md:text-base lg:px-6 lg:text-lg">Responsive Button</Button>
```
